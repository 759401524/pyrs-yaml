...
- **解析**了 YAML 文件，完整保留元数据
- 使用路径 API 和 Node API **查看**了值
- **编辑**了值、注释、锚点、标签和格式
- 使用 `set_many` **批量编辑**（通配符）
- 为可读性**排序**了键
- 根据 schema 进行了**校验**
- **复制**并**移动**了子树
- **序列化**回 YAML，所有内容都保留了下来

### 下一步

- [快速开始](../quick-start.md) — 几分钟内上手
- [就地编辑指南](../guides/editing.md) — 完整编辑 API 参考
- [自定义 Schema 指南](../guides/custom-schema.md) — 定义您自己的 schema
