...
graph TB
    ...
    subgraph Rust["Rust 레이어"]
        BIND["lib.rs<br/>PyO3 모듈 + 타입 변환"]
        AST["ast.rs<br/>CustomNode AST"]
        PARSER["parser/<br/>granit-parser 통합"]
...
graph TD
    ...
    C --> D["3. granit-parser → YAML 이벤트"]
    D --> E["4. AstReceiver가 CustomNode 구축"]
    E --> F["5. 스키마 타입 해석"]
    F --> G["6. 병합 키 해석 (활성화된 경우)"]
    G --> H["CustomNode (AST)"]
```
