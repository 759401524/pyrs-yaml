...
class TestI18N:
    ...
    def test_set_language(self):
        ...
        assert pyyaml_rs.get_language() == "en"

    def test_set_language_zh_cn(self):
        pyyaml_rs.set_language("zh-CN")
        assert pyyaml_rs.get_language() == "zh-CN"

    def test_set_language_ja_jp(self):
        pyyaml_rs.set_language("ja-JP")
        assert pyyaml_rs.get_language() == "ja-JP"
    ...
    def test_list_languages(self):
        langs = pyyaml_rs.list_languages()
    ...
    def test_negotiate_language_exact_match(self):
        result = pyyaml_rs.negotiate_language(["en"])
        assert result == "en"

    def test_negotiate_language_partial_match(self):
        result = pyyaml_rs.negotiate_language(["zh-CN", "en"])
        assert result in ("zh-CN", "en")

    def test_negotiate_language_fallback(self):
        result = pyyaml_rs.negotiate_language(["xx", "yy"], default="en")
        assert result == "en"

    def test_set_language_invalid(self):
        with pytest.raises(ValueError):
            pyyaml_rs.set_language("invalid_lang_xyz")
...
class TestParseAllDocs:
    ...
    def test_parse_all_docs_single(self):
        doc = pyyaml_rs.parse_all_docs("key: value")[0]
    ...
    def test_parse_all_docs_with_comments(self):
        docs = pyyaml_rs.parse_all_docs("# doc1\na: 1\n---\n# doc2\nb: 2")
        assert len(docs) == 2
...
class TestParseFile:
    ...
    def test_parse_file_nonexistent_raises(self):
        with pytest.raises(OSError):
            pyyaml_rs.parse_file("/nonexistent/path/to/file.yaml")
...
class TestToDict:
    ...
    def test_to_dict_with_list(self):
        doc = pyyaml_rs.parse("items:\n  - a\n  - b\n  - c")
        result = doc.to_dict()
        assert result == {"items": ["a", "b", "c"]}

    def test_to_dict_with_bool(self):
        doc = pyyaml_rs.parse("flag: true")
        result = doc.to_dict()
        assert result == {"flag": True}

    def test_to_dict_with_null(self):
        doc = pyyaml_rs.parse("key: null")
        result = doc.to_dict()
        assert result == {"key": None}

    def test_to_dict_with_anchor(self):
        doc = pyyaml_rs.parse("defaults: &d\n  timeout: 30\nprod:\n  <<: *d")
        result = doc.to_dict()
...
class TestYamlDocumentDunder:
    ...
    def test_root_type_alias(self):
        doc = pyyaml_rs.parse("base: &b val\nref: *b")
        assert doc.root_type() == "mapping"
...
class TestUnicodeAndSpecial:
    ...
    def test_unicode_cjk(self):
        doc = pyyaml_rs.parse("name: \u4e2d\u6587")
        assert doc.get("name") == "\u4e2d\u6587"

    def test_unicode_emoji(self):
        doc = pyyaml_rs.parse("emoji: \U0001f600")
...
class TestReadMarkdownErrors:
    ...
    def test_read_markdown_nonexistent_file(self):
        with pytest.raises(OSError):
            pyyaml_rs.read_markdown("/nonexistent/path/to/file.md")
...
class TestSafeLoad:
    ...
    def test_safe_load_with_bool_true(self):
        result = pyyaml_rs.safe_load("t: true")
        assert result["t"] is True

    def test_safe_load_with_bool_false(self):
        result = pyyaml_rs.safe_load("f: false")
...
class TestDumpFile:
    ...
    def test_dump_file_success(self):
        ...
        try:
            pyyaml_rs.dump_file(data, test_file)
            doc = pyyaml_rs.parse_file(test_file)
            assert doc.get("key") == "value"
    ...
    def test_dump_file_invalid_path_raises(self):
        with pytest.raises(OSError):
            pyyaml_rs.dump_file({"key": "value"}, "/nonexistent/dir/file.yaml")
...
class TestAnchorOnNonScalar:
    ...
    def test_anchor_on_mapping(self):
        doc = pyyaml_rs.parse("defaults: &defaults\n  timeout: 30\nhost: localhost")
        output = doc.to_yaml()
        assert "&defaults" in output

    def test_anchor_on_sequence(self):
        doc = pyyaml_rs.parse("items: &items\n  - a\n  - b\nref: *items")
        output = doc.to_yaml()
...
class TestSafeDumpTypes:
    ...
    def test_safe_dump_bool(self):
        output = pyyaml_rs.safe_dump({"flag": True})
        assert "flag: true" in output

    def test_safe_dump_null(self):
        output = pyyaml_rs.safe_dump({"key": None})
...
"""
    ...
    def test_yaml_suite_bool_true(self):
        for variant in ["true", "True", "TRUE"]:
            doc = pyyaml_rs.parse(f"key: {variant}")
