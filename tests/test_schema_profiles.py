...
class TestSchemaAliases:
    ...
    def test_yaml_org_2002(self):
        doc = pyrs_yaml.parse("x: 42", schema="yaml.org,2002")
        assert doc.get("x") == 42

    def test_json_tag_alias(self):
        doc = pyrs_yaml.parse("x: 0o10", schema="yaml.org,2002:json")
        assert doc.get("x") == "0o10"

    def test_failsafe_tag_alias(self):
        doc = pyrs_yaml.parse("x: true", schema="yaml.org,2002:failsafe")
        assert doc.get("x") == "true"

    def test_yaml11_tag_alias(self):
        doc = pyrs_yaml.parse("x: on", schema="yaml.org,2002:yaml1.1")
        assert doc.get("x") is True
    ...
    def test_1_1_alias(self):
        doc = pyrs_yaml.parse("x: yes", schema="1.1")
        assert doc.get("x") is True
...
class TestSchemaWithSafeLoad:
    ...
    def test_safe_load_yaml11(self):
        result = pyrs_yaml.safe_load("a: yes\nb: on", schema="yaml1.1")
        assert result["a"] is True
        assert result["b"] is True

    def test_safe_load_failsafe(self):
        result = pyrs_yaml.safe_load("a: true\nb: 42", schema="failsafe")
